...
- **Appairage par code 6 chiffres** : les étapes `user`,
  `zeroconf_confirm_auth` et l'OptionsFlow (Settings) demandent désormais un
  code à 6 chiffres affiché sur la TV (Service > Appairer), échangé contre la
  vraie clé API via `POST /api/pair`. La clé API n'est plus jamais saisie ni
  affichée en clair dans l'interface HA.
- Nouvelle erreur `pair_failed` (code invalide ou expiré), traduite dans les
  6 langues (`strings.json` + translations, libellés + `data_description`).
- Tests `config_flow` adaptés au nouveau contrat (51 tests verts).
