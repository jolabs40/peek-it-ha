...
# Import optionnel de adb-shell (ne casse pas l'intégration si absent)
...
try:
    ...
    _LOGGER.warning(
        "adb-shell non installé — les boutons ADB ne fonctionneront pas. "
        "Installer avec : pip install adb-shell"
    )
...
class PeekItAdbButton(ButtonEntity):
    ...
    async def _run_adb(self, commands: list[str]) -> bool:
        """Connexion ADB TCP et exécution des commandes shell."""
        if not _ADB_AVAILABLE:
            _LOGGER.error(
                "adb-shell non disponible — installer avec : pip install adb-shell"
            )
            return False
...
class PeekItEnableAssistButton(PeekItAdbButton):
    ...
    async def async_press(self) -> None:
        ...
        if ok:
            ...
            _LOGGER.error("Échec activation Assist sur %s", self._ip)
...
class PeekItDisableAssistButton(PeekItAdbButton):
    ...
    async def async_press(self) -> None:
        ...
        if ok:
            ...
            _LOGGER.error("Échec désactivation Assist sur %s", self._ip)
...
class PeekItEnableOverlayButton(PeekItAdbButton):
    ...
    async def async_press(self) -> None:
        ...
        if ok:
            ...
            _LOGGER.error("Échec activation overlay sur %s", self._ip)
...
class PeekItDisableOverlayButton(PeekItAdbButton):
    ...
    async def async_press(self) -> None:
        ...
        if ok:
            ...
            _LOGGER.error("Échec désactivation overlay sur %s", self._ip)
